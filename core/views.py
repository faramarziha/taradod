...
def management_dashboard(request):
    ...
    pending_edit_requests = (
        EditRequest.objects.filter(user__in=users_qs, status="pending")
        .select_related("user")
        .order_by("-created_at")[:5]
    )
    pending_leave_requests = (
        LeaveRequest.objects.filter(user__in=users_qs, status="pending")
        .select_related("user", "leave_type")
        .order_by("-created_at")[:5]
    )
    action_items = ([{"type": "edit", "obj": r} for r in pending_edit_requests] +
                    [{"type": "leave", "obj": r} for r in pending_leave_requests])
    action_items.sort(key=lambda x: x["obj"].created_at, reverse=True)

    # رادار عملکرد ماه گذشته
    ...
    for user in users_qs:
        ...
        logs = (
            AttendanceLog.objects.filter(
                user=user,
                timestamp__date__gte=start_period,
        ...
        for log in logs:
            ...
            if d not in first_logs:
                first_logs[d] = log
        tardies = 0
        on_time = 0
        for first_log in first_logs.values():
            if first_log.timestamp.time() > shift_start:
                tardies += 1
            else:
                on_time += 1
        if tardies:
            tardy_counts.append({"user": user, "count": tardies})
        if on_time:
            punctual_counts.append({"user": user, "count": on_time})

    tardy_leaderboard = sorted(tardy_counts, key=lambda x: x["count"], reverse=True)[:5]
    punctual_leaderboard = sorted(punctual_counts, key=lambda x: x["count"], reverse=True)[:5]
    ...
    context = {
        ...
        "tardy_leaderboard": tardy_leaderboard,
        "punctual_leaderboard": punctual_leaderboard,
        "device_online": device_online,
        "present_names_json": json.dumps([u.get_full_name() for u in present_users]),
        "absent_names_json": json.dumps([u.get_full_name() for u in absent_users]),
        "leave_names_json": json.dumps([u.get_full_name() for u in leave_users]),
