...
from attendance.models import AttendanceLog, SuspiciousLog, EditRequest, LeaveRequest, WeeklyHoliday, Shift, Group, LeaveType
from core.forms import CustomUserSimpleForm, InquiryForm, EditRequestForm, LeaveRequestForm, ManualLogForm, ManualLeaveForm, AttendanceStatusForm, UserLogsRangeForm, WeeklyHolidayForm, ShiftForm, GroupForm, LeaveTypeForm, ReportFilterForm, MonthlyPerformanceForm
...
def _calculate_monthly_performance(user, year, month):
    ...
    logs = list(AttendanceLog.objects.filter(user=user, timestamp__gte=global_start, timestamp__lt=global_end).order_by('timestamp'))
    weekly_holidays = set(WeeklyHoliday.objects.values_list('weekday', flat=True))
    leaves_qs = LeaveRequest.objects.filter(user=user, status='approved', start_date__lte=end_g, end_date__gte=start_g)
    leave_days = set()
    ...
    if shift_end <= shift_start:
        shift_minutes = (datetime.combine(start_g, shift_end) + timedelta(days=1) - datetime.combine(start_g, shift_start)).seconds // 60
    else:
        shift_minutes = (datetime.combine(start_g, shift_end) - datetime.combine(start_g, shift_start)).seconds // 60
    present_minutes = 0
    mandatory_minutes = 0
    tardy_minutes = 0
    overtime_minutes = 0
    ...
    for d in range(days_in_month):
        ...
        if current_in is not None:
            ...
            incomplete = True
        if not session_pairs and (not day_logs) and (not (prev_log and prev_log.log_type == 'in')):
            absence_days += 1
            continue
        if incomplete:
        ...
        if first_in_ts and first_in_ts > start_dt:
            tardy_minutes += int(math.ceil((first_in_ts - start_dt).total_seconds() / 60))
        for in_ts, out_ts in session_pairs:
            overlap_start = max(in_ts, start_dt)
            overlap_end = min(out_ts, end_dt)
            if overlap_start < overlap_end:
                present_minutes += int((overlap_end - overlap_start).total_seconds() // 60)
            if in_ts < start_dt:
                overtime_minutes += int((min(out_ts, start_dt) - in_ts).total_seconds() // 60)
            if out_ts > end_dt:
                overtime_minutes += int((out_ts - max(in_ts, end_dt)).total_seconds() // 60)
    report = {'present_minutes': present_minutes, 'mandatory_minutes': mandatory_minutes, 'tardy_minutes': tardy_minutes, 'absence_days': absence_days, 'present_hours': round(present_minutes / 60, 2), 'required_hours': round(mandatory_minutes / 60, 2), 'overtime_minutes': overtime_minutes, 'incomplete_days': [d for d in incomplete_days]}
...
def _get_face_encoding_from_base64(data_url: str):
    try:
        ...
        encs = face_recognition.face_encodings(np.array(img), num_jitters=5, model='large')
    ...
    try:
        ...
        for u in User.objects.exclude(face_encoding__isnull=True):
            ...
            if dist < 0.5:
                ...
                return JsonResponse({'ok': True, 'name': f'{u.first_name} {u.last_name}', 'code': u.personnel_code, 'timestamp': timezone.now().isoformat(), 'log_type': log_type, 'image_url': img_url})
        if best_user and best_dist < 0.6:
            try:
                ...
                log = SuspiciousLog.objects.create(matched_user=best_user, similarity=best_dist)
...
def user_inquiry(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                u = User.objects.get(personnel_code=cd['personnel_code'], national_id=cd['national_id'])
...
def user_profile(request):
    ...
    if not WeeklyHoliday.objects.filter(weekday=_weekday_index(today)).exists():
        ...
        elif LeaveRequest.objects.filter(user=u, status='approved', start_date__lte=today, end_date__gte=today).exists():
            status = 'leave'
        else:
            status = 'absent'
    ...
    monthly_stats = {'total_hours': report['present_hours'], 'total_delay': report['tardy_minutes'], 'absent_days': report['absence_days']}
    ...
    qs = AttendanceLog.objects.filter(user=u, timestamp__gte=start_dt, timestamp__lt=end_dt).order_by('timestamp')
    ...
    return render(request, 'core/user_profile.html', {'user': u, 'today_status': today_status, 'recent_events': events, 'monthly_stats': monthly_stats, 'daily_logs': daily_logs, 'log_jyear': ly, 'log_jmonth': lm, 'prev_month': f'{prev_m.year}-{prev_m.month:02d}', 'next_month': f'{next_m.year}-{next_m.month:02d}', 'edit_requests': edit_requests, 'leave_requests': leave_requests})
...
def leave_request(request):
    ...
    if request.method == 'POST':
        ...
        if 'confirm' in request.POST:
            ...
            return render(request, 'core/leave_request_confirm.html', {'form': form, 'user': u, 'end_date': form.cleaned_data['end_jalali']})
    else:
...
def management_users(request):
    ...
    if q:
        users = users.filter(Q(first_name__icontains=q) | Q(last_name__icontains=q) | Q(personnel_code__icontains=q))
    if status == 'active':
    ...
    ctx = {'users': users, 'groups': Group.objects.all(), 'shifts': Shift.objects.all()}
...
def admin_user_profile(request, pk):
    ...
    qs = AttendanceLog.objects.filter(user=user_obj, timestamp__date__range=(start_g, end_g)).order_by('timestamp')
    daily_logs = {d: {'in': None, 'out': None} for d in range(1, days + 1)}
    for log in qs:
        jd = jdatetime.date.fromgregorian(date=log.timestamp.date())
        info = daily_logs.get(jd.day)
    ...
    if requests_form.is_valid():
        ...
        if sd and ed:
            edit_requests = edit_requests.filter(created_at__date__gte=sd, created_at__date__lte=ed)
            leave_requests = leave_requests.filter(created_at__date__gte=sd, created_at__date__lte=ed)
    return render(request, 'core/admin_user_profile.html', {'user_obj': user_obj, 'form': form, 'requests_form': requests_form, 'edit_requests': edit_requests, 'leave_requests': leave_requests, 'daily_logs': daily_logs, 'log_jyear': ly, 'log_jmonth': lm, 'prev_month': f'{prev_m.year}-{prev_m.month:02d}', 'next_month': f'{next_m.year}-{next_m.month:02d}'})
...
def management_dashboard(request):
    ...
    today_in_logs = AttendanceLog.objects.filter(user__in=users_qs, timestamp__date=today, log_type='in').count()
    today_out_logs = AttendanceLog.objects.filter(user__in=users_qs, timestamp__date=today, log_type='out').count()
    if is_holiday:
        present_users = leave_users = absent_users = users_qs.none()
        present_ids = []
    else:
        present_ids = AttendanceLog.objects.filter(user__in=users_qs, timestamp__date=today).values_list('user_id', flat=True).distinct()
        leave_ids = LeaveRequest.objects.filter(user__in=users_qs, start_date__lte=today, end_date__gte=today).values_list('user_id', flat=True).distinct()
    ...
    for req in pending_edit_objs[:5]:
        pending_actions.append({'id': req.id, 'user': req.user, 'date': jdatetime.date.fromgregorian(date=req.timestamp.date()).strftime('%Y/%m/%d'), 'type': 'edit', 'type_label': f'ویرایش تردد ({req.get_log_type_display()})', 'action_url': reverse('edit_requests')})
    for req in pending_leave_objs[:5]:
        pending_actions.append({'id': req.id, 'user': req.user, 'date': jdatetime.date.fromgregorian(date=req.start_date).strftime('%Y/%m/%d'), 'type': 'leave', 'type_label': 'مرخصی', 'action_url': reverse('leave_requests')})
    ...
    for u in users_qs:
        ...
        for i in range(31):
            ...
            first_log = AttendanceLog.objects.filter(user=u, timestamp__date=day).order_by('timestamp').first()
            if first_log:
    ...
    context = {'active_tab': 'dashboard', 'today_in_logs': today_in_logs, 'today_out_logs': today_out_logs, 'present_users': present_users, 'absent_users': absent_users, 'leave_users': leave_users, 'present_count': present_users.count(), 'absent_count': absent_users.count(), 'leave_count': leave_users.count(), 'tardy_users': tardy_users, 'pending_edits': pending_edits, 'pending_leaves': pending_leaves, 'suspicious_today': suspicious_today, 'pending_actions': pending_actions, 'worst_performers': worst_performers, 'best_performers': best_performers, 'device_online': device_online, 'is_holiday': is_holiday, 'groups': Group.objects.all(), 'shifts': Shift.objects.all(), 'selected_group': group_id, 'selected_shift': shift_id}
...
def user_reports(request):
    ...
    context = {'active_tab': 'reports', 'active_users': active_users, 'inactive_users': inactive_users, 'no_face_users': no_face_users, 'logs': logs, 'form': form}
...
def monthly_profile(request):
    ...
    context = {'active_tab': 'reports', 'form': form, 'report': report, 'leaves': leaves, 'selected_user': selected_user}
...
def attendance_status(request):
    ...
    context = {'active_tab': 'attendance_status', 'present_users': present_users, 'absent_users': absent_users, 'leave_users': leave_users, 'jdate': jdate.strftime('%Y/%m/%d'), 'realtime': target_date == timezone.now().date(), 'form': form, 'holiday': holiday}
...
def api_attendance_status(request):
    ...
    if holiday:
        ...
        leave_users = User.objects.filter(id__in=leave_ids)
        absent_users = User.objects.filter(is_active=True).exclude(id__in=present_ids).exclude(id__in=leave_ids)
    data = {'present': [{'id': u.id, 'name': u.get_full_name(), 'code': u.personnel_code} for u in present_users], 'absent': [{'id': u.id, 'name': u.get_full_name(), 'code': u.personnel_code} for u in absent_users], 'leave': [{'id': u.id, 'name': u.get_full_name(), 'code': u.personnel_code} for u in leave_users]}
    return JsonResponse(data)

@login_required
@staff_required
def suspicious_logs(request):
    logs = SuspiciousLog.objects.select_related('matched_user').filter(status='pending').order_by('-timestamp')[:50]
    return render(request, 'core/suspicious_logs.html', {'active_tab': 'suspicions', 'logs': logs})
...
def edit_requests(request):
    ...
    if request.method == 'POST':
        ...
        if req.status == 'pending':
            ...
            if action == 'approve':
                AttendanceLog.objects.create(user=req.user, timestamp=req.timestamp, log_type=req.log_type, source='manager')
                req.status = 'approved'
                req.decision_at = timezone.now()
    ...
    return render(request, 'core/edit_requests.html', {'active_tab': 'edit_requests', 'requests': requests, 'groups': Group.objects.all(), 'shifts': Shift.objects.all(), 'selected_group': group_id, 'selected_shift': shift_id})
...
def leave_requests(request):
    ...
    if shift_id:
        requests = requests.filter(user__shift_id=shift_id)
    return render(request, 'core/leave_requests.html', {'active_tab': 'leave_requests', 'requests': requests, 'today': timezone.now().date(), 'groups': Group.objects.all(), 'shifts': Shift.objects.all(), 'selected_group': group_id, 'selected_shift': shift_id})
...
def add_log(request):
    ...
    if request.method == 'POST':
        ...
        form = ManualLogForm()
    return render(request, 'core/manual_log_form.html', {'active_tab': 'edit_requests', 'form': form})
...
def add_leave(request):
    ...
    return render(request, 'core/manual_leave_form.html', {'active_tab': 'leave_requests', 'form': form})
...
def user_logs_admin(request, user_id):
    ...
    return render(request, 'core/user_logs_admin.html', {'active_tab': 'management_users', 'user': user, 'form': form, 'logs': logs})
...
def shift_list(request):
    if not request.session.get('face_verified'):
        return redirect('management_face_check')
    shifts = Shift.objects.annotate(user_count=Count('customuser', distinct=True) + Count('groups__customuser', distinct=True))
...
def group_list(request):
    if not request.session.get('face_verified'):
        return redirect('management_face_check')
    groups = Group.objects.select_related('shift').annotate(user_count=Count('customuser')).all()
