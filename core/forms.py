...
from attendance.models import AttendanceLog, EditRequest, LeaveRequest, LOG_TYPE_CHOICES, WeeklyHoliday, Group, Shift
User = get_user_model()
...
class CustomUserSimpleForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ['username', 'first_name', 'last_name', 'personnel_code', 'national_id', 'group', 'shift', 'is_active', 'is_staff']
...
class EditRequestForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        if self.instance.pk and self.instance.timestamp:
            ...
            self.initial.setdefault('time', jd.time().replace(second=0, microsecond=0))
    ...
    def clean(self):
        ...
        if date and time_val:
            jdt = jdatetime.datetime(date.year, date.month, date.day, time_val.hour, time_val.minute, time_val.second)
...
class LeaveRequestForm(forms.ModelForm):
    ...
    leave_type = forms.ModelChoiceField(queryset=attendance_models.LeaveType.objects.all(), label='نوع مرخصی', required=False)

    class Meta:
        model = LeaveRequest
        fields = ['leave_type', 'start_date', 'duration', 'reason']
        labels = {'leave_type': 'نوع مرخصی', 'reason': 'توضیح'}
...
class ManualLogForm(forms.Form):
    ...
    def clean(self):
        ...
        if date_val and time_val:
            jdt = jdatetime.datetime(date_val.year, date_val.month, date_val.day, time_val.hour, time_val.minute)
    ...
    def save(self):
        return AttendanceLog.objects.create(user=self.cleaned_data['user'], timestamp=self.cleaned_data['timestamp'], log_type=self.cleaned_data['log_type'], source='manager')
...
class AttendanceStatusForm(forms.Form):
    date = jforms.jDateField(label='تاریخ', widget=JalaliDateInput(), required=False)
...
class WeeklyHolidayForm(forms.Form):
    days = forms.MultipleChoiceField(choices=[(0, 'شنبه'), (1, 'یکشنبه'), (2, 'دوشنبه'), (3, 'سه\u200cشنبه'), (4, 'چهارشنبه'), (5, 'پنجشنبه'), (6, 'جمعه')], widget=forms.CheckboxSelectMultiple, required=False, label='روزهای تعطیل')
...
class ShiftForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {'name': 'نام', 'start_time': 'شروع', 'end_time': 'پایان'}
        widgets = {'start_time': JalaliTimeInput(), 'end_time': JalaliTimeInput()}
...
class ReportFilterForm(forms.Form):
    start_date = jforms.jDateField(label='از تاریخ', widget=JalaliDateInput(attrs={'placeholder': '۱۴۰۳/۰۵/۱۰', 'class': 'date-input vjDateField', 'readonly': 'readonly'}), required=False)
    end_date = jforms.jDateField(label='تا تاریخ', widget=JalaliDateInput(attrs={'placeholder': '۱۴۰۳/۰۵/۱۰', 'class': 'date-input vjDateField', 'readonly': 'readonly'}), required=False)
    groups = forms.ModelMultipleChoiceField(queryset=Group.objects.all(), label='گروه\u200cها', required=False, widget=forms.SelectMultiple(attrs={'class': 'multi-select'}))
    shifts = forms.ModelMultipleChoiceField(queryset=Shift.objects.all(), label='شیفت\u200cها', required=False, widget=forms.SelectMultiple(attrs={'class': 'multi-select'}))
    users = forms.ModelMultipleChoiceField(queryset=User.objects.all(), label='کارکنان', required=False, widget=forms.SelectMultiple(attrs={'class': 'multi-select'}))

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['users'].label_from_instance = lambda obj: f'{obj.personnel_code} – {obj.first_name} {obj.last_name}'
